...
/*  31 */     super(ModTileEntities.TORIKAGO);
/*     */   }
/*     */ 
/*     */ 
/*     */   
...
/*  41 */       if (owner != null) {
/*     */         
/*  43 */         TorikagoAbility ability = (TorikagoAbility)AbilityDataCapability.get(owner).getEquippedAbility((Ability)TorikagoAbility.INSTANCE);
/*     */         
/*  45 */         if (ability != null) {
/*     */           
/*  47 */           double distance = getDistanceSq(owner.getPosX(), owner.getPosY(), owner.getPosZ());
/*  48 */           if (MathHelper.sqrt(distance) > (ability.roomSize + 2) || !ability.isContinuous()) {
...
/*  61 */     for (int i = -60; i < 60; i++) {
/*  62 */       for (int k = -60; k < 60; k++)
/*  63 */       { for (int j = -60; j < 60; j++)
/*  64 */         { if (this.world.getBlockState(new BlockPos(getPos().getX() + i, getPos().getY() + k, getPos().getZ() + j)).getBlock() == ModBlocks.STRING_WALL)
...
/* Location:              C:\Users\4tuto\curseforge\minecraft\Instances\incontrol\mods\mine-mine-no-mi-1.15.2-0.8.1.jar!\xyz\pixelatedw\mineminenomi\blocks\tileentities\TorikagoTileEntity.class

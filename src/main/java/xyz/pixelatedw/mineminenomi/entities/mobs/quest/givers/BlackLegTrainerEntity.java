...
public class BlackLegTrainerEntity extends TrainerEntity implements IHakiTrainer, IHakiUser {
  private static final String[] DEFAULT_TEXTURES = new String[] { "black_leg_trainer1", "black_leg_trainer2" };

  
  public BlackLegTrainerEntity(World world) {
    super(ModEntities.BLACK_LEG_TRAINER, world, DEFAULT_TEXTURES);
    EntityStatsCapability.get((LivingEntity)this).setFightingStyle("black_leg");
  }
  ...
  public void registerGoals() {
    ...
    this.targetSelector.addGoal(1, (Goal)new NearestAttackableTargetGoal(this, AbstractPirateEntity.class, true));
    this.targetSelector.addGoal(1, (Goal)new NearestAttackableTargetGoal(this, AbstractBanditEntity.class, true));
    this.targetSelector.addGoal(2, (Goal)new HurtByTargetGoal(this, new Class[0]));
    
    addBusoshokuHaki(this, 100);
  }
  ...
  public Quest[] getAvailableQuests(PlayerEntity player) {
    ...
    if (entityProps.isBlackLeg()) {
      availableQuests.addAll(Arrays.asList(ModQuests.BLACK_LEG_TRIALS));
    }

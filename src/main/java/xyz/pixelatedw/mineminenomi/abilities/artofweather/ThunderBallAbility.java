...
public class ThunderBallAbility extends Ability {
  ...
  private static final ParticleEffect CHARGE_PARTICLES = (ParticleEffect)new ChargedWeatherBallParticleEffect(WyHelper.hexToRGB("#FFFFFF"), ModParticleTypes.GORO_YELLOW);

  
  public ThunderBallAbility() {
    super("Thunder Ball", AbilityHelper.getStyleCategory());
    setDescription("Launch a Thunder Ball from your Clima Tact to use for different Tempos\n\n§2SHIFT-USE§r: Loads the ball into the clima tact");
    setMaxCooldown(1.0D);
  ...
  private boolean onUseEvent(PlayerEntity player) {
    ...
    if (player.isSneaking()) {
      ...
      ThunderBallProjectile proj = new ThunderBallProjectile(player.world, (LivingEntity)player);

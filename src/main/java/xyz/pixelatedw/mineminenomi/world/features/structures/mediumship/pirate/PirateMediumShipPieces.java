...
{
  private static final ResourceLocation PIECE = new ResourceLocation("mineminenomi", "pirate/medium_ship");
    ...
    extends TemplateStructurePiece
  {
    ...
    public Piece(TemplateManager template, CompoundNBT nbt) {
      super(ModFeatures.Pieces.PIRATE_MEDIUM_SHIP_PIECE, nbt);
    ...
    public Piece(TemplateManager template, ResourceLocation res, BlockPos pos, Rotation rot) {
      super(ModFeatures.Pieces.PIRATE_MEDIUM_SHIP_PIECE, 0);
      this.rotation = rot;
      this.resourceLocation = res;
      BlockPos blockpos = (BlockPos)PirateMediumShipPieces.POSITION_OFFSET.get(this.resourceLocation);
    ...
    private void build(TemplateManager templateManager) {
      ...
      PlacementSettings placementsettings = (new PlacementSettings()).setRotation(this.rotation).setMirror(Mirror.NONE).setCenterOffset((BlockPos)PirateMediumShipPieces.CENTER_OFFSET.get(this.resourceLocation)).addProcessor((StructureProcessor)BlockIgnoreStructureProcessor.AIR_AND_STRUCTURE_BLOCK);
    ...
    protected void handleDataMarker(String function, BlockPos pos, IWorld world, Random rand, MutableBoundingBox sbb) {
      if (function.equals("supplies_chest")) {
        StructuresHelper.spawnLoot(world, pos, ModLootTables.MEDIUM_PIRATE_SHIP_SUPPLIES_CHEST);
      }
      if (function.equals("food_chest")) {
        StructuresHelper.spawnLoot(world, pos, ModLootTables.MEDIUM_PIRATE_SHIP_FOOD_CHEST);
      }
      if (function.equals("captain_chest")) {
        StructuresHelper.spawnLoot(world, pos, ModLootTables.MEDIUM_PIRATE_SHIP_CAPTAIN_CHEST);
      }
      
      if (function.equals("deck_spawn")) {
        StructuresHelper.spawnMobs(world, pos, StructuresHelper.StructureFaction.PIRATE, StructuresHelper.StructureSpawnType.GRUNT, 3);
      }
    ...
    public boolean create(IWorld world, ChunkGenerator<?> generator, Random random, MutableBoundingBox bb, ChunkPos chunkPos) {
      PlacementSettings placementsettings = (new PlacementSettings()).setRotation(this.rotation).setMirror(Mirror.NONE).setCenterOffset((BlockPos)PirateMediumShipPieces.CENTER_OFFSET.get(this.resourceLocation)).addProcessor((StructureProcessor)BlockIgnoreStructureProcessor.AIR_AND_STRUCTURE_BLOCK);
      BlockPos offset = (BlockPos)PirateMediumShipPieces.POSITION_OFFSET.get(this.resourceLocation);

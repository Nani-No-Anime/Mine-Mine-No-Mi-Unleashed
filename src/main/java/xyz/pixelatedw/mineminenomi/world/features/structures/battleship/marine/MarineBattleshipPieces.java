...
{
  private static final ResourceLocation PIECE = new ResourceLocation("mineminenomi", "marine/battle_ship");
    ...
    extends TemplateStructurePiece
  {
    ...
    public Piece(TemplateManager template, CompoundNBT nbt) {
      super(ModFeatures.Pieces.MARINE_BATTLESHIP_PIECE, nbt);
    ...
    public Piece(TemplateManager template, ResourceLocation res, BlockPos pos, Rotation rot) {
      super(ModFeatures.Pieces.MARINE_BATTLESHIP_PIECE, 0);
      this.rotation = rot;
      this.resourceLocation = res;
      BlockPos blockpos = (BlockPos)MarineBattleshipPieces.POSITION_OFFSET.get(this.resourceLocation);
    ...
    private void build(TemplateManager templateManager) {
      ...
      PlacementSettings placementsettings = (new PlacementSettings()).setRotation(this.rotation).setMirror(Mirror.NONE).setCenterOffset((BlockPos)MarineBattleshipPieces.CENTER_OFFSET.get(this.resourceLocation)).addProcessor((StructureProcessor)BlockIgnoreStructureProcessor.AIR_AND_STRUCTURE_BLOCK);
    ...
    protected void handleDataMarker(String function, BlockPos pos, IWorld world, Random rand, MutableBoundingBox sbb) {
      if (function.equals("supplies_chest")) {
        StructuresHelper.spawnLoot(world, pos, ModLootTables.LARGE_MARINE_SHIP_SUPPLIES_CHEST);
      }
      if (function.equals("captain_chest")) {
        StructuresHelper.spawnLoot(world, pos, ModLootTables.LARGE_MARINE_SHIP_CAPTAIN_CHEST);
      }
      
      if (function.equals("deck_spawn")) {
        StructuresHelper.spawnMobs(world, pos, StructuresHelper.StructureFaction.MARINE, StructuresHelper.StructureSpawnType.GRUNT, 10);
      }
      if (function.equals("back_deck_spawn")) {
        StructuresHelper.spawnMobs(world, pos, StructuresHelper.StructureFaction.MARINE, StructuresHelper.StructureSpawnType.BOMBER, 2);
      }
      if (function.equals("tower_spawn")) {
        StructuresHelper.spawnMobs(world, pos, StructuresHelper.StructureFaction.MARINE, StructuresHelper.StructureSpawnType.SNIPER, 2);
      }
      ...
      if (function.equals("cannons_spawn_2")) {
        ...
        StructuresHelper.StructureSpawnType type = StructuresHelper.StructureSpawnType.CAPTAIN;
        int number = 1;
        if (world.getRandom().nextFloat() >= 0.4F) {
          
          type = StructuresHelper.StructureSpawnType.BOMBER;
          number = 2;
        } 
        StructuresHelper.spawnMobs(world, pos, StructuresHelper.StructureFaction.MARINE, type, number);
      } 
    }
    ...
    public boolean create(IWorld world, ChunkGenerator<?> generator, Random random, MutableBoundingBox bb, ChunkPos chunkPos) {
      PlacementSettings placementsettings = (new PlacementSettings()).setRotation(this.rotation).setMirror(Mirror.NONE).setCenterOffset((BlockPos)MarineBattleshipPieces.CENTER_OFFSET.get(this.resourceLocation)).addProcessor((StructureProcessor)BlockIgnoreStructureProcessor.AIR_AND_STRUCTURE_BLOCK);
      BlockPos offset = (BlockPos)MarineBattleshipPieces.POSITION_OFFSET.get(this.resourceLocation);

  ...
  extends OPStructure<NoFeatureConfig>
{
  ...
  public String getStructureName() {
    return (new ResourceLocation("mineminenomi", "ghost_ship")).toString();
  }
  ...
  public boolean canBeGenerated(BiomeManager biomeManager, ChunkGenerator<?> chunkGen, Random rand, int chunkX, int chunkZ, Biome biome) {
    boolean canSpawn = CommonConfig.INSTANCE.canSpawnGhostShips();
    boolean isFlat = WyHelper.isSurfaceFlat(chunkGen, chunkX, chunkZ, 2);
    boolean isChance = (MathHelper.clamp(WyHelper.randomWithRange(0, 100) + WyHelper.randomDouble(), 0.0D, 100.0D) < CommonConfig.INSTANCE.getChanceForGhostShipSpawn());
  ...
  public static void register(Biome biome) {
    if (!CommonConfig.INSTANCE.canSpawnGhostShips()) {
      return;
    }
    if (biome.getCategory() == Biome.Category.OCEAN) {
      
      Structure structure = ModFeatures.GHOST_SHIP;
    ...
    extends StructureStart
  {
    ...
    public void init(ChunkGenerator<?> generator, TemplateManager templateManagerIn, int chunkX, int chunkZ, Biome biome) {
      int i = chunkX * 16;
      int j = chunkZ * 16;
      BlockPos blockpos = new BlockPos(i, generator.getSeaLevel() - 6, j);
      Rotation rotation = Rotation.values()[this.rand.nextInt((Rotation.values()).length)];
      GhostShipPieces.addComponents(templateManagerIn, blockpos, rotation, this.components);
      recalculateStructureSize();
      StructuresHelper.SPAWNED_STRUCTURES.add(blockpos);
      StructuresHelper.STRUCTURES_COUNT[4] = StructuresHelper.STRUCTURES_COUNT[4] + 1;
      
      WyDebug.debug("Ghost Ship spawned at: /tp " + blockpos.getX() + " ~ " + blockpos.getZ());
    }
  }
}

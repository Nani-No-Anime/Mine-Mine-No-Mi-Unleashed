...
/* 28 */     LiteralArgumentBuilder<CommandSource> builder = (LiteralArgumentBuilder<CommandSource>)Commands.literal("belly").requires(source -> source.hasPermissionLevel(2));
/*    */     
/* 30 */     builder
/* 31 */       .then(((RequiredArgumentBuilder)Commands.argument("amount", (ArgumentType)LongArgumentType.longArg(-999999999L, 999999999L))
/* 32 */         .executes(context -> alterBelly(context, LongArgumentType.getLong(context, "amount"), getDefaultCollection(context))))
/* 33 */         .then(Commands.argument("targets", (ArgumentType)EntityArgument.players())
/* 34 */           .executes(context -> alterBelly(context, LongArgumentType.getLong(context, "amount"), EntityArgument.getPlayers(context, "targets")))));
/*    */     
...
/*    */       
/* 50 */       entityStatsProps.alterBelly(amount);
/* 51 */       WyNetwork.sendTo(new SSyncEntityStatsPacket(player.getEntityId(), entityStatsProps), (PlayerEntity)player);
/*    */       
/* 53 */       ((CommandSource)context.getSource()).sendFeedback((ITextComponent)new StringTextComponent(TextFormatting.GREEN + "" + ((amount > 0L) ? "+" : "") + amount + " belly for " + player.getDisplayName().getFormattedText()), true);
/*    */     } 
/*    */ 
/*    */     
/* 57 */     return 1;
...
/* Location:              C:\Users\4tuto\curseforge\minecraft\Instances\incontrol\mods\mine-mine-no-mi-1.15.2-0.8.1.jar!\xyz\pixelatedw\mineminenomi\commands\BellyCommand.class

...
public class ExtolCommand {
  public static void register(CommandDispatcher<CommandSource> dispatcher) {
    LiteralArgumentBuilder<CommandSource> builder = (LiteralArgumentBuilder<CommandSource>)Commands.literal("extol").requires(source -> source.hasPermissionLevel(2));
    
    builder
      .then(((RequiredArgumentBuilder)Commands.argument("amount", (ArgumentType)LongArgumentType.longArg(-999999999L, 999999999L))
        .executes(context -> alterExtol(context, LongArgumentType.getLong(context, "amount"), getDefaultCollection(context))))
        .then(Commands.argument("targets", (ArgumentType)EntityArgument.players())
          .executes(context -> alterExtol(context, LongArgumentType.getLong(context, "amount"), EntityArgument.getPlayers(context, "targets")))));
  ...
  private static int alterExtol(CommandContext<CommandSource> context, long amount, Collection<ServerPlayerEntity> targets) throws CommandSyntaxException {
    for (ServerPlayerEntity player : targets) {
      ...
      entityStatsProps.alterExtol(amount);
      WyNetwork.sendTo(new SSyncEntityStatsPacket(player.getEntityId(), entityStatsProps), (PlayerEntity)player);
      
      ((CommandSource)context.getSource()).sendFeedback((ITextComponent)new StringTextComponent(TextFormatting.GREEN + "" + ((amount > 0L) ? "+" : "") + amount + " extol for " + player.getDisplayName().getFormattedText()), true);
    } 
